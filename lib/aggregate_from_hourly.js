...
var areatypes = require('geo_bbox').areatypes
...
exports.aggregates = function aggregates(options){
    ...
    return function aggregates(next){
        ...
        var file_q = async.queue(file_worker, 10)
    ...
    return function(err,text){
        ...
        // iterate over each group
            ...
            _.each(intermediate,function(value,key){
                ...
                var n = value.length;
                var j = _.keys(unmapper).length
                for(var i = startidx;i<j;i++){
                    start.push(0);
                }
                start[unmapper['detectors']]=[]
                //console.log(value[0])
                //console.log(start)
                var printix = 0;
                var end = _.reduce(value
                              ,function(memo, rec){
                                  ...
                                  _.each(sum_variables,function(variable){
                                      // if(variable === 'detector_count'){
                                      //     console.log([memo[unmapper[variable]], rec[unmapper[variable]]])
                                      // }
                                  ...
                                  _.each(avg_nhh_vars,function(variable){
                                   ...
                                   // array concatenation
                                   memo[unmapper['detectors']] = memo[unmapper['detectors']].concat(rec[unmapper['detectors']])
                                   return memo;
                               }
                                  , start);
                _.each(avg_hh_vars,function(variable){
            ...
            //console.log('\n divide by '+end[unmapper['n']]);
                ...
                end[unmapper['detectors']] = _.unique(end[unmapper['detectors']])
                aggoutput.features[0].properties.data.push( _.flatten(end) );
            ...
            if(fwyagg){
                ...
                var intermediate = _.groupBy(aggoutput.features[0].properties.data,grouper)
                aggoutput.features[0].properties.data = [];
                ...
                _.each(intermediate,function(value,key){
                    ...
                    // initialize the rest with zeros
                    var n = value.length;
                    var j = _.keys(unmapper).length + 1 // deleted freeway, but need to account for it still
                    for(var i = startidx;i<j;i++){
                        start.push(0);
                    }
                    start[unmapper['detectors']]=[]
