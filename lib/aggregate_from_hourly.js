...
exports.aggregates = function aggregates(options){
    ...
    function checkfs (queue,done){
        // populate the file combinations array
        // url and file pattern is
        // /data/{area}/{timeagg}/{year}/{filename}

        async.each(years,function(year,cb1){
            async.each( _.keys(areatypes), function(area,cb2){
                console.log(area)
    ...
    return function aggregates(next){
        ...
        checkfs(file_q,function(e){
            // assign a callback
            console.log(file_q.length()+' items')
