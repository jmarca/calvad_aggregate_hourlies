...
exports.aggregates = function aggregates(options){
    ...
    function file_worker(task,done){
        //console.log('file_worker for '+task.aggfile)
        var q = queue(1)
        q.defer(check_dir,task)
        q.defer(check_file,task)
        q.defer(aggregate,task)
        q.await(function(e,r){
            //console.log('file_worker finished for '+task.aggfile)
            if(e) console.log(e)
            return done(e,task.aggfile)
        })
        return null
    }

    function check_dir(task,callback){
    ...
    function statfile (hfile,area,year,file,workerqueue,cb){
        var split_file = file.split('.')
        fs.stat(hfile,function(err,stats){
            if(err){
                console.log('no source hourly file',hfile)
            }else{
                ...
                _.each(other_aggregations,function(agg){
                    _.each([false,true],function(fwy){
                        ...
                        workerqueue.push({'aggpath':aggpath
                                          ,'hourlyfile':hfile
                                          ,'agg':agg
                                          ,'aggfile':aggfile})
                        return null
    ...
    return function aggregates(next){
        ...
        // and another queue to run stat
        var q = queue(1)

        // all the file combinations
        _.each(years,function(year,cb1){
            _.each( _.keys(areatypes), function(area,cb2){
                console.log(area)
                var files = areatypes[area]
                _.each(files
                       ,function(file){
                           ...
                           q.defer(statfile,hourlyfile,area,year,file,workerqueue)
                           return null
                       })
                return null
            })
        ...
        q.awaitAll(function(e,r){
            console.log('done with checking queue')
            console.log(workerqueue.length + ' jobs to process')
            if(e) throw new Error(e)
            // finished all possible defers for workerqueue, so assign a final job
            var jobq = queue(jobs)
            workerqueue.forEach(function(job){
                jobq.defer(file_worker,job)
                return null
            })
            jobq.awaitAll(function(e,r){
                console.log('all items have been processed '+r.length + ' jobs done')
                if(next){
                    next();
                }
                return null
...
function aggregate(options,cb){
    ...
    if(!options.doit){
        ...
        fs.readFile(options.hourlyfile,{encoding:'utf8'},
                    aggregator(options.agg,options.aggfile,cb));
    }
    return null
}
...
function aggregator(agg,aggpath,cb){
    ...
    return function(err,text){
        ...
        // console.log('write out '+aggpath)
        fs.writeFile(aggpath,JSON.stringify(aggoutput),function(err){

            console.log('finished writing out '+aggpath)
            if(err){
